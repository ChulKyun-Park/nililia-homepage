...
| 서비스 상세 페이지 | `/services/video-translation` 등 8개 |
...
| 컴포넌트 | 파일 | 사용처 |
|---------|------|--------|
| `PageHero` | `src/components/sections/PageHero.tsx` | 메뉴 페이지 + 서비스 상세 페이지 히어로 |
...
| `BrochureModal` | `src/components/brochure/BrochureModal.tsx` | 회사소개서 다운로드 팝업 |
| `ContactForm` | `src/components/contact/ContactForm.tsx` | 문의하기 폼 |

### 서비스 상세 페이지 5-섹션 구조

```
PageHero (좌 텍스트 + 우 이미지)
...
1. 공통 컴포넌트로 해결 가능한가? (가능하면 개별 파일 수정 금지)
2. 홈 탭 기준과 일치하는가?
3. TypeScript + CSS만 사용했는가?
4. `npm run build` 에러 없는가?
5. 모바일 375px 레이아웃 정상인가?
